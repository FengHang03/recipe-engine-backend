...
from app.L2Generator.l2_data_models import (
    PetProfile, L2Input,
    OptimizationResult, SolveStatus, 
)
...
class RecipeEngine:
    ...
    def generate_recipes(self, pet_profile: PetProfile, top_k: int = 5) -> List[OptimizationResult]:
        ...
        # L2 批量计算
        for i, comb in enumerate(candidates):
            ...
            # 进度日志
            if (i + 1) % 50 == 0:
                logger.info(f"已优化 {i + 1}/{len(candidates)} 个组合，找到 {len(results)} 个可行解")
    ...
    def _is_quality_recipe(self, result: OptimizationResult) -> bool:
        # 补剂总重量不能超过 5%
        if result.total_weight_grams < 1e-6:
            return False
