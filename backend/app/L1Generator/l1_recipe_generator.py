...
# 导入配置
from app.L1Generator.l1_config import (
    L1Config,
    SlotConfig,
    IngredientFilter,
    ExclusionRule,
...
from app.L1Generator.l1_slot_scheduler import (
    SlotScheduler
)
...
class CombinationGenerator:
    ...
    def _record_used_ids(self, current_combo: Dict[str, List[Ingredient]]):
        """提取当前组合中所有食材 ID 并更新到全局集合"""
        # 扁平化提取并直接填充字典
        new_data = {
            ing.ingredient_id: ing
            for ing_list in current_combo.values() 
            for ing in ing_list
        }
        # update 会自动处理：已有的覆盖（引用不变），没有的新增
        self.used_ingredients.update(new_data)
    ...
    def _backtrack(
        ...
        if not remaining_slots:
            ...
            if combo is not None:
                ...
                self._record_used_ids(current_combo)
