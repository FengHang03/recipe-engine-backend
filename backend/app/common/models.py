...
class Ingredient(BaseModel):
    ...
    def in_food_group(self, groups: List[FoodGroup]) -> bool:
        return self.food_group in [g.value for g in groups]

class RecipeCombination(BaseModel):
    """L1 输出的候选组合"""
    recipe_id           : str
    ingredients         : Dict[str, List[Ingredient]] = Field(default_factory=dict) # slot_name -> [ingredients]
