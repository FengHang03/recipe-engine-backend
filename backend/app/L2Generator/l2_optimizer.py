...
# from app.L2Generator.test_nutrition import nutrient_sum
from app.common.utils import (
    UnitConverter
)
from app.common.enums import LifeStage, NutrientID, SlotType
from app.common.models import RecipeCombination, Ingredient
from app.database import data_loader
from app.database.data_loader import IngredientDataLoader
from .l2_data_models import (
    PetProfile, L2Input, get_nutrient_value,
...
class L2Optimizer:
    ...
    def _solve_phase(
        ...
        energy_map = l2_input.nutrient_matrix[NutrientID.ENERGY].to_dict()
        ...
        # 设置超时 (10 秒)
        self.solver.SetTimeLimit(100000)
    ...
    def _build_objective(
        ...
        # ===== 5. 槽位理想范围罚分 (软约束) =====
        slot_grouped_ingredients = l2_input.combination.ingredients
        slot_penalty = self._build_slot_penalty(slot_grouped_ingredients)
    ...
    def _build_slot_penalty(self, slot_grouped_ingredients: Dict[str, List[Ingredient]]):
        """
        构建槽位理想范围罚分 (软约束)
        ...
        for slot_name, ings in slot_grouped_ingredients.items():
            constraint = get_slot_constraint(slot_name)
            if not constraint:
                continue
                
            # 预先计算该槽位下所有成分变量的和
            weight_sum = sum(self.vars[ing.ingredient_id] for ing in ings)
            
            # 构建软约束 (Penalty Logic)
            if constraint.ideal_min:
                dev_min = self.solver.NumVar(0, self.solver.infinity(), f'{slot_name}_dev_min')
                # weight_sum < ideal_min * total -> 产生惩罚
                self.solver.Add(dev_min >= constraint.ideal_min * self.total_weight_var - weight_sum)
                # penalty += dev_min * constraint.penalty_weight # 建议增加权重系数
                
            if constraint.ideal_max:
                dev_max = self.solver.NumVar(0, self.solver.infinity(), f'{slot_name}_dev_max')
                # weight_sum > ideal_max * total -> 产生惩罚
                self.solver.Add(dev_max >= weight_sum - constraint.ideal_max * self.total_weight_var)
                # penalty += dev_max * constraint.penalty_weight
    ...
    def _parse_result(
        ...
        # 2. 准备数据（防污染）
        # -------------------------
        df = l2_input.converted_nutrient_matrix.copy()
        df = df.set_index('ingredient_id', drop=True, inplace=False) if 'ingredient_id' in df.columns else df

        # -------------------------
        # 3. 构建 weight 向量（核心优化）
        # -------------------------
        weight_dict = {
            ing.ingredient_id: self.vars[ing.ingredient_id].solution_value()
            for ing in ingredients
        }
        weight_series = pd.Series(weight_dict)
        weight_series = weight_series[weight_series >= 0.1] # 过滤极小值

        # -------------------------
        # 4. 对齐矩阵
        # -------------------------
        df = df.loc[df.index.intersection(weight_series.index)]
        weight_series = weight_series.loc[df.index]

        # -------------------------
        # 5. 向量化营养计算（核心🔥）
        # -------------------------
        nutrient_cols = df.columns
        scaled_matrix = df.mul(weight_series, axis=0) / 100
        nutrient_sum_dict = scaled_matrix[nutrient_cols].sum().to_dict()

        energy_key = NutrientID.ENERGY.value
        total_calorie = nutrient_sum_dict.get(energy_key, 0)

        # 防止除零
        if total_calorie <= 0:
            raise ValueError("Total calorie is zero or invalid")

        nutrient_sum_dict = {
            k: v / total_calorie * 1000
        ...
        for ing in ingredients:
            weight = weight_dict.get(ing.ingredient_id, 0)
            if weight < 0.1:
                continue
