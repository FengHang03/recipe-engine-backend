...
from app.DataProcessor.dataset_preparing_refactored import ThresholdValue
from app.common.enums import FoodGroup, FoodSubgroup, LifeStage, NutrientID
...
class IngredientDataLoader:
    ...
    def get_nutrition_unit_factor(self, nutrient_info: pd.DataFrame, life_stage: LifeStage = LifeStage.DOG_ADULT) -> Dict[int, float]:
        ...
        nutrient_info : pd.DataFrame
            营养素信息数据表，需包含以下列（列名支持灵活匹配）：
            - nutrient_id/index: 营养素ID（int，二选一存在即可）
            - unit_name: 营养素当前单位名称（如"G"、"MG"、"KG/100G"）
        life_stage : LifeStage, optional
            宠物生命阶段枚举（默认：成年犬 DOG_ADULT），用于匹配对应AAFCO标准
        ...
        Dict[int, float]
            营养素单位转换系数映射字典：
            - Key: 营养素ID（int）
            - Value: 转换系数（float），1.0表示无需转换（无标准/转换失败）
        """
        factors_map = {}
    ...
    def get_converted_nutrition_matrix(self, nutrient_matrix: pd.DataFrame, factor_map: Dict[int, float]) -> pd.DataFrame:
        """
        获取与AAFCO转换因子相乘后的营养矩阵

        Args:
        nutrient_matrix: 原始宽表 (values 可能混杂 mg, ug, IU)
        factor_map: 转换单位
        """
        converted_matrix = nutrient_matrix.copy().fillna(0)
        numeric_cols = converted_matrix.select_dtypes(include=['int', 'float']).columns.drop("ingredient_id", errors="ignore")
        converted_matrix[numeric_cols] = converted_matrix[numeric_cols].fillna(0)

        for col in converted_matrix.columns:
            ...
            if pd.api.types.is_numeric_dtype(converted_matrix[col]):
                converted_matrix[col] = converted_matrix[col] * factor
...
if __name__ == "__main__":
    ...
    converted_nutrient_matrix = loader.get_converted_nutrition_matrix(nutrition_matrix, nut_conv_factors)
    # converted_nutrient_matrix.to_csv("converted_matrix.csv")

    for ing_id in selected_ids:
        
        converted_nutrient_matrix.loc[ing_id] = converted_nutrient_matrix.loc[ing_id] * 1.5 / 100

    # converted_nutrient_matrix.to_csv("converted_matrix_1.csv")
